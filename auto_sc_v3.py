...
class PowerAnalyzerGUI:
    ...
    def build_gui(self):
        ...
        self.device_entries = {}
        self.device_status_labels = {}  # 상태 레이블 저장용
        ...
        # 상태 출력용 Label 추가
            status_label = tk.Label(self.root, text="미연결", fg="gray")
            status_label.grid(row=i, column=3, sticky="w")
            self.device_status_labels[label] = status_label
        
        # build_gui 함수 내 온도 센서 라벨 선언 부분 밑에 추가
        tk.Label(self.root, text="온도 센서 채널:").grid(row=4, column=2)
        self.temp_channel_entry = tk.Entry(self.root, width=10)
        self.temp_channel_entry.grid(row=4, column=3)
        self.temp_channel_entry.insert(0, "1")
    ...
    def connect_all(self):
        for label, entry in self.device_entries.items():
            ...
            status_label = self.device_status_labels[label]

            if not address:
                status_label.config(text="주소 없음", fg="gray")
                continue

            try:
                idn_info = None  # 기기 정보 저장용
                if address.upper().startswith("USB") or "::" in address:
                    inst = self.rm.open_resource(address)
                    inst.timeout = 3000
                    idn_info = inst.query("*IDN?").strip()
                    self.instrument[label] = inst
                    self.connection_type[label] = "VISA"
                    self.log(f"{label} 연결 성공 (VISA): {idn_info}")
                    status_label.config(text=f"연결됨 - {idn_info}", fg="green")
                    ...
                    # SERIAL은 보통 IDN? 명령이 없음, 직접 수동 지정도 가능
                    idn_info = f"{address} (Serial)"
                    self.log(f"{label} 연결 성공 (SERIAL): {idn_info}")
                    status_label.config(text=f"연결됨 - {idn_info}", fg="green")
                    ...
                    try:
                        sock.sendall(b"*IDN?\n")
                        # sock.sendall(b"*INFO?\n")
                        recv_data = sock.recv(1024).decode().strip()
                        idn_info = recv_data if recv_data else f"{ip}:{port}"
                    except Exception:
                        idn_info = f"{ip}:{port} (LAN)"
                    self.log(f"{label} 연결 성공 (LAN): {idn_info}")
                    status_label.config(text=f"연결됨 - {idn_info}", fg="green")
    ...
    def start_test(self):
        ...
        try:
            self.temp_channel = int(self.temp_channel_entry.get())
        except Exception as e:
            self.log("온도 센서 채널 입력 오류: {}".format(e))
            messagebox.showerror("채널 입력 오류", str(e))
            return
                ...
                while time.time() - start_time < self.wait_time:
                    ...
                    try:
                        ...
                        if temp_label in self.instrument:
                            channel_cmd = f":VAL{self.temp_channel}:VAL?"
                            val = self.query(temp_label, channel_cmd)
                            if val is not None:
